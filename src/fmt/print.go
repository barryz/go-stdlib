...
// State表示了传递给自定义formatters的priter状态.
type State interface {
	// Write is the function to call to emit formatted output to be printed.
	// Write 函数用于打印出已经格式化的输出:w
	Write(b []byte) (n int, err error)
	// Width returns the value of the width option and whether it has been set.
	// Width 返回宽度选项值，以及它是否被设置
	Width() (wid int, ok bool)
	// Precision returns the value of the precision option and whether it has been set.
	// Precision 返回精度选项值，以及它是否被设置
...
// such as Print.
// 根据Go语言中实现接口的定义，一个类型只要有 `String() string` 方法，
// 我们就说它实现了Stringer接口。而在本节开始已经说到，
...
// to a %#v format.
// 如果一个类型有`Gostring() string`方法，那么它就实现了GoStringer接口
// GoString函数定义了该值在Go中的语法表示
// 如果在格式化输出中有`%#v`格式化字符串，那么将会调用Gostring()方法进行处理
type GoStringer interface {
	GoString() string
}

// Use simple []byte instead of bytes.Buffer to avoid large dependency.
// 使用[]byte来代替bytes.Buffer避免大量的依赖
type buffer []byte

// 下面的四个方法模拟了bytes.Buffer中常用的四个方法
...
// pp 用来存储printer的状态，并通过sync.Pool重用，以避免分配
type pp struct {
