...
//
// strings包实现了一些函数用来操作UTF-8编码后的字符串
...
// 返回子串substr在s中出现的个数（不重叠的）
// 如果子串为空， 该函数返回s中的Unicode代码点数+1
// 调用了Index函数
func Count(s, substr string) int {
	...
	if len(substr) == 1 {
		return bytealg.CountString(s, substr[0]) // 当子串长度为1时， 使用bytealg中的CountString方法计算子串个数
	}
	n := 0
	for {
		i := Index(s, substr) // 获取子串在s中的索引位置
		if i == -1 {          // 索引为负数
			return n // 返回count
		}
		n++                   // 自增count
		s = s[i+len(substr):] // 更新子串，字符串切片，复杂度O(1)
	}
}
